...
import { selectedBook } from "../../redux/booklist/actions";
import deleteBook from "../../redux/booklist/thunk/deleteBooks";
import "../../styles/style.css";

const Book = ({ book }) => {
  const dispatch = useDispatch();
  const { id, name, author, thumbnail, price, featured } = book;
      ...
      <div className="book-card">
        ...
        <div className="flex-1 h-full pr-2 pt-2 flex flex-col">
          <div className="flex items-center justify-between">
            <span
              className={`${
                JSON.parse(featured) ? "badge-success" : ""
              } lws-Badge `}
            >
              {JSON.parse(featured) ? "Featured" : ""}
            </span>
          ...
          <div className="space-y-2 mt-4 h-full">
            ...
            <p className="lws-price">USD {price}</p>
          </div>
        </div>
      </div>
    </div>
